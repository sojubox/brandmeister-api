...
def json(f):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not isinstance(rv, dict):
            # assume it is a model and call its export_data() method
            rv = rv.export_data()
    ...
    def decorator(f):
        ...
        def wrapped(*args, **kwargs):
            if current_app.config['USE_RATE_LIMITS']:
                # Generate a unique key to represent the decorated function and the IP address of the client.
                # Rate limiting counters are maintained on each unique key
                key = '{0}/{1}'.format(f.__name__, str(g.user.id))
                limiter = RateLimit(key, limit, period)

                # Set the rate limit headers in g, so that they are picked up by the after_request handler and
                # attached to the response
                g.headers = {
                    'X-RateLimit-Remaining': str(limiter.remaining if limiter.remaining >= 0 else 0),
                    'X-RateLimit-Limit': str(limit),
                    'X-RateLimit-Reset': str(limiter.reset)
                }

                # If the client went over the limit respond with a 429 status code, else invoke the wrapped function
...
# This decorator adds an ETag header to the GET/HEAD responses
def etag(f):
    @functools.wraps(f)
    def wrapped(*args, **kwargs):
...
def _filter_query(model, query, filter_spec):
    ...
    for f in filters:
        ...
        if f[1] == 'in':
            f = [f[0], f[1], f[2:]]
        ops = {'eq': '__eq__', 'ne': '__ne__', 'lt': '__lt__', 'le': '__le__', 'gt': '__gt__', 'ge': '__ge__',
               'in': 'in_', 'like': 'like'}
...
# This decorator implements pagination, filtering, sorting and expanding for collections. The expected response from
# the decorated route is a SQLAlchemy query.
def collection(model, name=None, max_per_page=10):
    ...
    def decorator(f):
        ...
        def wrapped(*args, **kwargs):
            ...
            if p.has_prev:
                pages['prev_url'] = url_for(request.endpoint, page=p.prev_num, per_page=per_page,
                                            expand=expand, _external=True, **kwargs)
            else:
                pages['prev_url'] = None

            if p.has_next:
                pages['next_url'] = url_for(request.endpoint, filter=filter, sort=sort, page=p.next_num,
                                            per_page=per_page, expand=expand, _external=True, **kwargs)
            else:
                pages['next_url'] = None
                pages['first_url'] = url_for(request.endpoint, filter=filter, sort=sort, page=1, per_page=per_page,
                                             expand=expand, _external=True, **kwargs)
